...
function swMainStart(e) {
    if (unlocked && parseInt(st.style.left) > PAD + 10) return;
    drag = true;
    ox = e.touches ? e.touches[0].clientX : e.clientX;
    startLeft = parseInt(st.style.left) || PAD;
    st.style.transition = 'none'; sf.style.transition = 'none';
...
function lock() {
    ...
    st.style.left = PAD + 'px'; sf.style.width = '0%';
    charImg.style.opacity = "0";
    isMindClicked = false;
    mindTrigger.style.color = "#fff";
    document.querySelectorAll('.btn, #switchBtn').forEach(b => b.classList.remove('revealed'));
}
